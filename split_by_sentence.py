...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for input_data in data.iterrows():
                ...
                with autocast():
                    ...
                    if tok.seperation_mark in chunk_y_label:
                        for seperation_index in np.where(np.array(chunk_y_label) == '%')[0]:
                            ...
                            write(os.path.join(config.music_dir,input_data[1]['video_name'] + '_'+ str(cnt) +'.wav'),sr,mel_to_audio(x[:,:,last_index:seperation_frame].tolist(),sr=22050,n_fft=1024,hop_length=256))
            ...
            if chunk_y_label[-1] != '%':
                write(os.path.join(config.music_dir,input_data[1]['video_name'] + '_'+ str(cnt) +'.wav'),sr,mel_to_audio(x[:,:,last_index:].tolist(),sr=22050,n_fft=1024,hop_length=256))
                new_video_name.append(input_data[1]['video_name'] + '_'+ str(cnt) +'.wav')
                new_lyrics.append(lyrics[cnt])
                cnt += 1 
