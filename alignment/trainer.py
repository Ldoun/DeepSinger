...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        with torch.autograd.set_detect_anomaly(True):
            with autocast():
                while chunk_index < (engine.max_target_ratio * max(y_length.tolist())) -1 :      
                    engine.model.train()
                    ...
                    total_acc += (y_hat.argmax(1).view(1,-1) == chunk_y_label.view(-1)).sum().item()
    ...
    def validate(engine, mini_batch):
        with torch.no_grad():
            ...
            with autocast():
                ...
                total_acc += (y_hat.argmax(1).view(1,-1) == y.view(-1)).sum().item()
